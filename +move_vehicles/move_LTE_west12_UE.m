function move_LTE_west12_UE(LTE_config,UEs,networkPathlossMap,eNodeBs)
...
y_range=[-ISD/sqrt(3),-ISD/sqrt(3)/2-2*lane_width];  % [-288m,-151m] (road line distance=137m )
...
%% special 1+1 vehicles are moving to north direction 
for u_ =53
    UEs(u_).move;        
    ...
    if ROI_teleport || handover_requested 
        ...
        if ROI_teleport
            ...
            new_UE_position = [x_init, y_range(2)-50-1];%when vehicles go out, come in from opposite direction
...
for u_ =62
    ...
    if ROI_teleport || handover_requested 
        ...
        if ROI_teleport
            ...
            new_UE_position = [x_init, y_range(1)-50-1];%when vehicles go out, come in from opposite direction
