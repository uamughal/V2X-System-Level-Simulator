function move_LTE_north_UE(LTE_config,UEs,networkPathlossMap,eNodeBs)
...
x_range=[-ISD/3,0];  % [-167m,0m] (road line distance=167m )
...
%% 3+3 vehicles are moving to south direction 
for u_ =77:79
    UEs(u_).move;    
    ...
    if ROI_teleport || handover_requested 
        ...
        if ROI_teleport
            ...
            y_init=ISD/sqrt(3)-lane_width/2; 
...
for u_ =80:82
    ...
    if ROI_teleport || handover_requested 
        ...
        if ROI_teleport
            ...
            y_init=ISD/sqrt(3)-lane_width*3/2; 
            new_UE_position = [x_range(1)+1, y_init];%when vehicles go out, come in from opposite direction
