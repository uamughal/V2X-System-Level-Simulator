function move_LTE_south_UE(LTE_config,UEs,networkPathlossMap,eNodeBs)
...
x_range=[ISD/2.0.*(-1)+2*lane_width,ISD/3.0.*(1/2)-2*lane_width];  % [-243m,76.3m] (road line distance=319m )
 
 % If a UE went outside of ROI, relocate him from the begaining of the train line. 
% distance_resolution_x=(x_range(2)-x_range(1))/(LTE_config.simulation_time_tti/LTE_config.TTI_resolution);


%% First 6+6 vehicles are moving to west direction 
for u_ =21:26
    UEs(u_).move;  
    ...
    if ROI_teleport || handover_requested 
        ...
        if ROI_teleport
            ...
            y_init=-ISD/sqrt(3)/2+lane_width*3/2; 
            new_UE_position = [x_range(2)-1, y_init];%when vehicles go out, come in from opposite direction
...
for u_ =27:32
    ...
    if ROI_teleport || handover_requested
        ...
        if ROI_teleport
            ...
            y_init=-ISD/sqrt(3)/2+lane_width/2;
            new_UE_position = [x_range(2)-1, y_init];  %when vehicles go out, come in from opposite direction
...
%% Second 6+6 vehicles are moving to east direction 
for u_ =33:38
    UEs(u_).move;
    ...
    if ROI_teleport || handover_requested 
        ...
        if ROI_teleport
            ...
            y_init=-ISD/sqrt(3)/2-lane_width/2;
            new_UE_position = [x_range(1)+1, y_init];  %when vehicles go out, come in from opposite direction
