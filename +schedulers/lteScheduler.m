    ...
    methods
        ...
        function [assigned_CQI predicted_BLER predicted_SINR_dB predicted_Is predicted_Is_min] = get_optimum_CQIs(obj,CQIs_to_average_all)
            % Preallocation
        ...
        function schedule_users_common(obj,attached_UEs,UE_feedback,current_TTI,tx_mode)
            ...
            for u_=1:length(attached_UEs)
                ...
                if UE_feedback.feedback_received(u_)
                    ...
                    if (tx_mode == 4) || (tx_mode == 3)  % SM modes use the RI value also
                        if tx_mode==4
                            PMI = UE_feedback.PMI(:,u_);
                        end
                        nLayers    = UE_feedback.RI(u_);
                        nCodewords = min(2,nLayers);     % Supports 4 layers also
                    else
                        nLayers    = 1;
                        nCodewords = 1;
                    end
                    ...
                    [num_assigned_RB, CQIs_to_average_all, UE_scheduled, RB_map] = obj.filter_out_zero_RBs_and_get_CQIs(the_RB_grid,nCodewords,UE_CQI_feedback,current_UE);
                    
                    if UE_scheduled
                        % Simplified this piece of code by using the superclass, as all types of scheduler will to make use of it.
                        [assigned_CQI, predicted_UE_BLERs(1:nCodewords,u_), estimated_TB_SINR, Is, I_min] = obj.get_optimum_CQIs(CQIs_to_average_all);
                        ...
                        if tx_mode==4
                            the_RB_grid.PMI(RB_map) = PMI(RB_map);
                        else
                            the_RB_grid.PMI(RB_map) = NaN;
                        end
                    else
                    ...
                    nLayers         = 1;
                    if tx_mode==4
                        the_RB_grid.PMI(RB_map) = 1;   % A random precoder. BEWARE: one-indexed!! In the standard, it is zero-indexed
                    else
                ...
                if UE_scheduled
                    ...
                    if strcmp(class(obj.attached_eNodeB.scheduler),'schedulers.ffrScheduler') % mod(current_TTI-1,5) ||  TB without sync symbols. Solution (for now) for FFR
