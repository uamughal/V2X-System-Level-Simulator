...
% (c) Josep Colom Ikuno, INTHFT, 2008
   ...
   properties
       ...
       eNodeB_tx_traces
       % Traces of what the center UE eNodeB sends
       UE_eNodeB_tx_traces
       % Traces of the UEs feedback
       eNodeB_rx_feedback_traces
       % Traces of the broadcast UEs feedback
       UE_eNodeB_rx_feedback_traces
   ...
   methods
       ...
       % for Vehicle UE
        function calculate_broadcast_UE_aggregates(obj)
           for s_=1:length(obj.VeNodeB_tx_traces)
               obj.UE_eNodeB_tx_traces(s_).calculate_final_average_BLER;
           end
           for u_=1:length(obj.UE_traces)
               obj.UE_traces(u_).calculate_final_average_throughput_Mbps;
               obj.UE_traces(u_).calculate_final_average_spectral_efficiency_bit_per_cu;
