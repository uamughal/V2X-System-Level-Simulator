function LTE_plot_loaded_network(LTE_config,eNodeBs,eNodeBs_sectors,UEs,networkPathlossMap,CQI_mapper,use_subplots,networkShadowFadingMap)
% Function that shows a few plots after a network has been loaded from a file
% (c) Josep Colom Ikuno, INTHFT, 2008
% www.nt.tuwien.ac.at

if ~isempty(networkShadowFadingMap)
...
antenna_types =  cell(1,length(eNodeBs_sectors));
if LTE_config.show_network>0
    for i_=1:length(eNodeBs_sectors) % Needed because Matlab does not have and built-in class-casting yet
        antenna_types{i_} = eNodeBs_sectors(i_).antenna.antenna_type;
    ...
    for ant_idx = 1:length(anntenna_types)
        ...
        an_antenna = eNodeBs_sectors(example_ant_idx).antenna;
...
if LTE_config.show_network>0 && LTE_config.macroscopic_pathloss_is_model
    ...
    for s_=1:length(eNodeBs_sectors)
        if ~isempty(eNodeBs_sectors(s_).macroscopic_pathloss_model)
            all_pathloss_model_names{sIdx} = eNodeBs_sectors(s_).macroscopic_pathloss_model.name; %#ok<AGROW>
            sIdx = sIdx + 1;
        end
    end
    if ~isempty(all_pathloss_model_names)
        [unique_pathlosses m n] = unique(all_pathloss_model_names); %#ok
        % Will set the maximum distance as the diagonal that crosses the
        % ROI  sqrt(x*x+y*y)
        range = sqrt((roi_to_map_x(2)-roi_to_map_x(1))^2+(roi_to_map_y(2)-roi_to_map_y(1))^2);
        distances = 0:LTE_config.map_resolution:range;
        
        if use_subplots
            figure(LTE_config.plots.macroscopic_pathloss); % LTE_config.plots.macroscopic_pathloss==4(you can find in parameters)
        end
        for i_=1:length(m)
            if use_subplots
            ...
            macroscopic_pathloss_model = eNodeBs_sectors(m(i_)).macroscopic_pathloss_model;
...
if LTE_config.show_network>0
    number_rows = 3;
    number_cols = 4;
    num_figures = ceil(length(eNodeBs_sectors)/(number_rows*number_cols));
    ...
    for f_=0:(num_figures-1)
        ...
        for subplot_idx = 1:(number_rows*number_cols)
            if b_ <= length(eNodeBs_sectors)
                parent_eNodeB = eNodeBs_sectors(b_).parent_eNodeB;
                ...
                imagesc(networkPathlossMap.roi_x,networkPathlossMap.roi_y,networkPathlossMap.pathloss(:,:,b_));
                set(gca,'YDir','normal');
                title(sprintf('eNodeB %d sector %d',parent_eNodeB.id,eNodeBs_sectors(b_).id));
                colorbar;
                hold on;
                ...
                for bIdx=1:length(eNodeBs)
                    if eNodeBs(bIdx).id~=b_
                        scatter(eNodeBs(bIdx).pos(1),eNodeBs(bIdx).pos(2),'.','MarkerEdgeColor',0.75*[1 1 1]);
                    end
                end
                b_ = b_ + 1;
            end
...
if LTE_config.show_network>0 && plot_shadow_fading && ~isa(networkShadowFadingMap,'channel_gain_wrappers.shadowFadingDummyMap')
...
if LTE_config.show_network>0
    if plot_shadow_fading
        plot_SINR_plots(LTE_config,eNodeBs,eNodeBs_sectors,UEs,networkPathlossMap,CQI_mapper,1,use_subplots);
        plot_SINR_plots(LTE_config,eNodeBs,eNodeBs_sectors,UEs,networkPathlossMap,CQI_mapper,2,use_subplots);
    else
        plot_SINR_plots(LTE_config,eNodeBs,eNodeBs_sectors,UEs,networkPathlossMap,CQI_mapper,3,use_subplots);
    end
end

function plot_SINR_plots(LTE_config,eNodeBs,eNodeBs_sectors,UEs,networkPathlossMap,CQI_mapper,mode,use_subplots)
...
for u_ = 1:length(UEs)
    scatter(UEs(u_).pos(1),UEs(u_).pos(2),'.','m'); % purple
  %  text(UEs(u_).pos(1)+7*networkPathlossMap.data_res,UEs(u_).pos(2),num2str(u_),'Color','w');
end
%% Plot CQIs
if use_subplots
    subplot(2,2,2);
