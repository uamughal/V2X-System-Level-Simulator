function [bulk_parameters] = generate_bulk_par(wimpar,linkpar,fixpar)
...
%
%   WINNER Phase II interim (WIM2i): Pekka Kyösti (EBIT), Lassi Hentilä (EBIT),
%   Marko Milojevic (TUI), Mikko Alatossava (CWC/UOULU), Martin Käske (TUI)
%
%   WINNER Phase I (WIM): Daniela Laselva (EBIT), Marko Milojevic (TUI),
%   Pekka Kyösti (EBIT), Lassi Hentilä (EBIT)
%
...
NumOfScenarios = 15;    % should be equal to the number in ScenarioMapping.m
...
for ScenIndex = 1:NumOfScenarios
    ...
    if iterpar.UserIndeces
        switch iterpar.Scenario
            % A1, B1, C2, D1, Geometric based stochastic models
            case {'A1', 'A2', 'B1', 'B2', 'B3', 'B4', 'C1', 'C2', 'C3', 'C4', 'D1', 'D2a'}
                bulk_parameters_iter = stochastic(wimpar,linkpar,fixpar,iterpar);

                % B5, Static feeder scenario
            case {'B5a','B5b','B5c','B5f'}
                bulk_parameters_iter = static(wimpar,linkpar,fixpar,iterpar);
...
% See [1, Sec.4.2].
function bulk_parameters=stochastic(wimpar,linkpar,fixpar,iterpar)
...
if strcmpi(FixedPdpUsed,'no')
    ...
    switch upper(Scenario)      % See distributions in [1, table 4-5]
        ...
        case {'B1','B2'}
            ...
            %for NLoS links
            if NLoSConnectionLinks
                taus(NLoSConnectionLinks,1:N(2)) = sort(800E-9*rand(NumNLoSConnectionLinks,N(2)),2);  % Uni(0,800ns)
            end
...
if strcmpi(FixedPdpUsed,'no')
    ...
    if NLoSConnectionLinks
        if strcmpi(Scenario,'B1')
            ...
            Pprime(NLoSConnectionLinks,1:N(2)) = exp(-taus_without_los_factor(NLoSConnectionLinks,1:N(2))./sigma_ds(NLoSConnectionLinks,1:N(2))).*10.^(-ksi_NLoS/10);
        else
...
if strcmpi(wimpar.FixedAnglesUsed,'no')
    ...
    ConstantC = [4 5 8 10 11 12 14 15 16 20;...
    ...
    if LoSConnectionLinks
        ...
        AoD_path(LoSConnectionLinks,1:N(1)) = (AoDPrimer(LoSConnectionLinks,1:N(1)).*(2*round(rand(NumLoSConnectionLinks,N(1)))-1) + ...
            repmat(sigma_asD(LoSConnectionLinks),1,N(1))/1.4/5.*randn(NumLoSConnectionLinks,N(1))) - ...
            (repmat(AoDPrimer(LoSConnectionLinks,1),1,N(1)).*(2*round(rand(NumLoSConnectionLinks,N(1)))-1) + ...
            repmat(sigma_asD(LoSConnectionLinks),1,N(1))/1.4/5*randn(1,1) - ...
            repmat(ThetaBs(LoSConnectionLinks).',1,N(1))); %[1, eq. 4.13]
        ...
        AoA_path(LoSConnectionLinks,1:N(1)) = (AoAPrimer(LoSConnectionLinks,1:N(1)).*(2*round(rand(NumLoSConnectionLinks,N(1)))-1) + ...
            repmat(sigma_asA(LoSConnectionLinks),1,N(1))/1.4/5.*randn(NumLoSConnectionLinks,N(1))) - ...
            (repmat(AoAPrimer(LoSConnectionLinks,1),1,N(1)).*(2*round(rand(NumLoSConnectionLinks,N(1)))-1) + ...
            repmat(sigma_asA(LoSConnectionLinks),1,N(1))/1.4/5*randn(1,1) - ...
            repmat(ThetaMs(LoSConnectionLinks).',1,N(1))); %[1, eq. 4.13]
...
% See [1, Sec.6.12].
function bulk_parameters=static(wimpar,linkpar,fixpar,iterpar)
