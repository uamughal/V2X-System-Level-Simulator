...
%
%   See also LAYOUTPARSET.

% Authors: Daniela Laselva (EBIT), Milan Narandži? (TUI)

function NTlayout(layoutpar)

linkpar=layout2link(layoutpar);

NofBs=length(linkpar.NofSect);
NofSectTot=sum(linkpar.NofSect);
NofStatTot=length(linkpar.Stations);
NofMs=NofStatTot-NofSectTot;
BsIdx=[1 cumsum(linkpar.NofSect)+1]; BsIdx(end)=[];
BsXY=[linkpar.Stations(BsIdx).Pos];           % coordinates of BSs
BsXY(3,:)=[];
MsIdx=NofSectTot+1:NofStatTot;
MsXY=[linkpar.Stations(MsIdx).Pos];           % coordinates of BSs
MsXY(3,:)=[];
...
% 
% for sn=1:NofSectTot
%     rv=rotate_vector([0 1 0]',linkpar.Stations(sn).Rot);
%     BsOmega(sn)=90-mycart2sph(rv(1),rv(2),rv(3))*180/pi;;
% end

flag=1; % if 1 draw Bs broadside, if 2 active links, if 3 Ms'directions

% Get number of links and Ms & Sector indices            
K = size(Pairing,2);    % number of links
%[r c] = find(Pairing);
%indMs = intersect(Pairing(:),MsIdx)-NofSectTot;             % links coupling to MSs
indMs = Pairing(2,:)-NofSectTot;             % links coupling to MSs

%indSect = intersect(Pairing(:),1:NofSectTot);           % links coupling to Sectors (1:NofSect*NofBs)
indSect = Pairing(1,:);           % links coupling to Sectors (1:NofSect*NofBs)

% Get links coupling to BSs
tmpsum = cumsum(NofSect);  
for k=1:length(indSect) 
    tmp = find(diff(indSect(k)<=tmpsum));
...
for sn=1:length(indSect)
    rv=rotate_vector([0 1 0]',linkpar.Stations(indSect(sn)).Rot);
    BsOmega(indSectors(sn),indBs(sn))=90-mycart2sph(rv(1),rv(2),rv(3))*180/pi;
end
%Append BsOmega for sectors without links
indBsU=unique(indBs);
BsIdxE=[BsIdx NofSectTot+1];
for bsn=1:length(indBsU)
    SecRange=BsIdxE(bsn):BsIdxE(bsn+1)-1;
    tmp=intersect(SecRange,indSect); %sectors with links
    ci=length(tmp)+1;
    missingSect=setdiff(SecRange,tmp);
    for msn=1:length(missingSect)
        rv=rotate_vector([0 1 0]',linkpar.Stations(missingSect(msn)).Rot);
        BsOmega(ci,indBsU(bsn))=90-mycart2sph(rv(1),rv(2),rv(3))*180/pi;
        ci=ci+1;
    end
end
...
xlabel('Cells area, X[m]','FontSize',12); ylabel('Cells area, Y[m]','FontSize',12);
...
for i=1:NofBs 
    n=NofSect(i); 
    for nn=1:n 
        arrow(BsXY,MsXY,indBs,indMs,BsOmega,nn,i,flag);...
    end,
end
...
if link                        % link=1 if a link exists 
    ...
    OmegaTmp=Omega(nn,i); 
end % Ms not linked to any Bs, or Bs not covering any link
%OmegaTmp=Omega(nn,i);
...
hsiz = 0.08; %0.2;   % Size arrow head vs length of the vector
hwid  = 0.15; %0.5;  % Width of the arrow head base vs length
