classdef eNodeB_sector < handle
    ...
    methods
        ...
        function receive_UE_feedback(obj)
         ...
         % Receives and stores the received feedbacks from the UEs
        function receive_UE_broadcast_feedback(obj)
            max_streams  = 2;
            obj.last_received_feedback.UE_id             = zeros(length(obj.connected_UEs),1);
            obj.last_received_feedback.tx_mode           = zeros(length(obj.connected_UEs),1); % From what mode the CQI and RI was taken
            obj.last_received_feedback.nCodewords        = zeros(length(obj.connected_UEs),1); % Relates to the ACK/NACK report. For the 0-delay case, this and tx_mode could differ (ACK is always delayed)
            obj.last_received_feedback.CQI               = zeros(50,max_streams,length(obj.connected_UEs));
            obj.last_received_feedback.RI                = zeros(length(obj.connected_UEs),1);
            obj.last_received_feedback.PMI               = nan(50,length(obj.connected_UEs));
            obj.last_received_feedback.feedback_received = false(length(obj.connected_UEs),1);
            UE_feedback_idx = 1;

            for i_=1:length(obj.connected_UEs)
                UE_i  = obj.connected_UEs(i_);
                
                % Receive the feedback from each user
                UE_id       = UE_i.id;
                feedback_u_ = UE_i.side_uplink_channel.get_feedback;

                    %feedback_u_.TTI_idx = zeros(1,1);
                    %feedback_u_.tx_mode = zeros(1,1);
                    %feedback_u_.nCodewords = zeros(1,1);
                    %feedback_u_.CQI = zeros(2,50);
                    %feedback_u_.ACK = zeros(1,2);
                    %feedback_u_.PMI = zeros(1,50);
                    %feedback_u_.RI = zeros(1,1);
                    %feedback_u_.TB_size = zeros(1,2);
                    %feedback_u_.UE_scheduled = zeros(1,2);                    
                    %UE_id = 0;
                    %obj.parent_eNodeB.id = 0; 


               % feedback_u_ = obj.broadcast_feedback_buffer(i_).get_broadcast_feedback; %
                % The first TTI, even with 0 delay there is no feedback, as no ACKs are available
                if ~isempty(feedback_u_)
                    % For the zero delay case, substitute the delayed feedback with a zero-delay CQI and (if applicable), RI feedback.
                    % The rest of the feedback data is delayed 1 TTI (no ACK is possible before the reception is done)
        ...
        function schedule_users(obj)
           ...
           % obj.attached_CoMP_site.scheduler.schedule_users()
            % Store traces
