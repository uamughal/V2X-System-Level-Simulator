classdef UE < handle
    ...
    properties
        ...
        side_downlink_channel  % sidelink downlink model for this user
        ...
        side_uplink_channel    % Sidelink upwnlink model for this user
        ...
        % Add eNB function to the Vehicle UEs
        sites
        sectors
        neighbors_UEs
        connected_UEs
        ...
        broadcast_UEs
        broadcast_trace
        broadcast_trace_SINR
        VeNodeB_signaling
        broadcast_RB_grid
        broadcast_traffic_model
        side_link_quality_model_output
        scheduler
    ...
    methods
        ...
        function clear(obj)
            ...
            obj.side_downlink_channel     = [];
            obj.RB_grid                   = [];
            obj.uplink_channel            = [];
            obj.side_uplink_channel       = [];
            obj.trace                     = [];
            obj.clock                     = [];
            obj.CQI_mapper                = [];
        ...
        function send_broadcast_feedback(obj)
            obj.side_uplink_channel.send_broadcast_feedback(obj.feedback);
        end
 ...
 %% Generate Pathloss for link Vehicle to Vehicle
                ...
                MsHeight = 1.5;
                CenterFrequency = 6e9;
                % UE_distance=sqrt((obj.pos(1)-obj.attached_UE.pos(1)).*(obj.pos(1)-obj.attached_UE.pos(1))...
                 %            +(obj.pos(2)-obj.attached_UE.pos(2)).*(obj.pos(2)-obj.attached_UE.pos(2)));
                % Dist1  = UE_distance; 
                Dist1=120;
                ...
                loss_LoS = [];               
%                 if Dist1 <= PL_bp && Dist1 >= 10
%                     loss_LoS = 22.7*log10(Dist1) + 41.0 + 20*log10(CenterFrequency/5);
%                 end
%                 if Dist1 >= PL_bp && Dist1 <= 5000
                    loss_LoS = 40*log10(Dist1) + 9.45 - 17.3*log10(H_bs) - 17.3*log10(H_ms) + 2.7*log10(CenterFrequency/5);
%                 end  
                if Dist1 < 10
                    loss_LoS = 0;
                end
                user_macroscopic_pathloss_dB  = loss_LoS;   
                
%% Get Shadowing Fading
                user_shadow_fading_loss_dB   = obj.downlink_channel.shadow_fading_pathloss;
              %  there_are_interferers        = ~isempty(interfering_eNodeBs);
                there_are_interferers        = false;
            else
                UE_id       = obj.id;
        ...
        function [interfering_macroscopic_pathloss_eNodeB_dB, interfering_shadow_fading_loss_dB]...
            ...
            if ~obj.trace_UE
                interfering_macroscopic_pathloss_eNodeB_dB = obj.downlink_channel.interfering_macroscopic_pathloss(interfering_eNodeB_ids)- obj.antenna_gain;
        ...
        function link_quality_model(obj,config)
            ...
            %assume winner fast fading include pathloss + shadowing + fastfading
            ...
            [zeta,chi,psi] = obj.side_downlink_channel.fast_fading_model.generate_fast_fading_signal(t,tx_mode);
            ...
            if there_are_interferers % no interfering eNodeBs present (single eNodeB simulation)
                ...
                total_RX_Power   = 0.5*log10(sum(RX_total_RB));
                totalInterfPower = 0.5*log10(reshape(sum(interfering_power_allocations,1),[],1))-interfering_macroscopic_pathloss_eNodeB_dB-interfering_shadow_fading_loss_dB;
            ...
            obj.SNR_avg_preequal = 2*log10(mean(RX_total)./thermal_noise_watts_per_half_RB); % mean over the subcarriers
          ...
          if there_are_interferers
                obj.wideband_SINR = 2*log10(sum(RX_total(:))/(sum(interf_power_all_RB(:))+thermal_noise_watts_per_half_RB*nSC));
                wide_interference=2*log10((sum(interf_power_all_RB(:))+thermal_noise_watts_per_half_RB*nSC));
                else
                obj.wideband_SINR = 2*log10(sum(RX_total(:))/(thermal_noise_watts_per_half_RB*nSC));
                 wide_interference=2*log10(thermal_noise_watts_per_half_RB*nSC);
          end
            ...
            % SIR_dB  = 10*log10(SIR_linear);
               %% for CS to  557
        if isfield(config,'CoMP_configuration')
            if tx_mode==1
                n_interferer=length(interfering_eNodeBs); %intererence eNodeB
            ...
            kk=wideband_SIR<config.SIR_threshold;
          
            %% SINR_recalculation for CS
            if sum(kk)~=0
               switch tx_mode
                case 1
                    % SINR calculation (SISO)
                    ...
                    if there_are_interferers
                        % Also works for more than one rank (i.e. extra dimension)
                        % mute the interferenc for CoMP eNB
                        interf_power_all_RB_repmat=interf_power_all_RB_repmat(~kk,:);
                        theta=theta(~kk,:);
                        interfering_rx_power = squeeze(sum(interf_power_all_RB_repmat.*theta,1));
                        Interference_plus_noise_power = noise_plus_inter_layer_power + interfering_rx_power.';
                    else
              ...
              if there_are_interferers
                 interf_power_all_RB=interf_power_all_RB(:,~kk);
        ...
        function link_performance_model(obj)
            ...
            [...
               ...
               N_used_bits,...
               packet_parts...
               ] = obj.attached_UE.eNodeB_signaling.get_TB_params;
        ...
        function process_packet_parts(obj,packet_parts,nCodewords)
            for cw_ = 1:nCodewords
                if obj.feedback.ACK(cw_)
                    if strcmp(obj.traffic_model.type,'voip') || strcmp(obj.traffic_model.type,'video') || strcmp(obj.traffic_model.type,'CAM')
