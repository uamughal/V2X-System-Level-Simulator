classdef plotUtils
    ...
    methods(Static)
        ...
        function figure_handle = plot_sector_SINR_cdfs(figure_handle,networkPathlossMap,networkShadowFadingMap, LTE_config, eNodeBs_sectors)
            ...
            if ~isempty(networkPathlossMap.SINR2)
                use_shadowing = true;
            else
                use_shadowing = false;
            end
            ...
            if use_shadowing
                if consider_all_map
                    ...
                    shadowing_assignment     = false(size(networkPathlossMap.sector_assignment{1}));
                    non_shadowing_assignment = false(size(networkPathlossMap.sector_assignment2{1}));
                    for i_=1:length(considered_eNodeBs)
                        shadowing_assignment = shadowing_assignment|(networkPathlossMap.sector_assignment{1}==considered_eNodeBs(i_));
                        non_shadowing_assignment = non_shadowing_assignment|(networkPathlossMap.sector_assignment2{1}==considered_eNodeBs(i_));
                    end
                    ...
                    all_pathloss = networkPathlossMap.pathloss;
                    all_shadow   = zeros([size(all_pathloss,1) size(all_pathloss,2) nSites]);
                    for s_=1:nSites
                        all_shadow(:,:,s_) = imresize(networkShadowFadingMap.pathloss(:,:,s_),[size(all_pathloss,1),size(all_pathloss,2)]);
                    end
                    all_pathloss_plus_shadow = all_pathloss;
                    for s_=1:length(eNodeBs_sectors)
                        all_pathloss_plus_shadow(:,:,s_) = all_pathloss_plus_shadow(:,:,s_) + all_shadow(:,:,eNodeBs_sectors(s_).parent_eNodeB.id);
                    end
                    
                    % Cut out the unneeded parts and generate a combined pathloss map
                    combined_PL_shadowing    = zeros(size(networkPathlossMap.sector_assignment{1}));
                    combined_PL_no_shadowing = zeros(size(networkPathlossMap.sector_assignment2{1}));
                    for s_=1:length(eNodeBs_sectors)
                        current_PL_no_shadowing = all_pathloss(:,:,s_);
                        current_PL_shadowing    = all_pathloss_plus_shadow(:,:,s_);
                        current_PL_no_shadowing(networkPathlossMap.sector_assignment2{1}~=s_) = NaN;
                        current_PL_shadowing(networkPathlossMap.sector_assignment{1}~=s_) = NaN;
                        
                        combined_PL_shadowing(networkPathlossMap.sector_assignment{1}==s_)     = current_PL_shadowing(networkPathlossMap.sector_assignment{1}==s_);
                        combined_PL_no_shadowing(networkPathlossMap.sector_assignment2{1}==s_) = current_PL_no_shadowing(networkPathlossMap.sector_assignment2{1}==s_);
                        all_pathloss(:,:,s_)             = current_PL_no_shadowing;
                        all_pathloss_plus_shadow(:,:,s_) = current_PL_shadowing;
                    end
                    
                    [f3, x3] = ecdf(reshape(all_pathloss_plus_shadow(:,:,considered_eNodeBs),1,[]));
                    [f4, x4] = ecdf(reshape(all_pathloss(:,:,considered_eNodeBs),1,[]));
                    ...
                    if extra_pathloss_plots
                        ...
                        RX_power_shadow    = TX_power-pLoss-reshape(all_pathloss_plus_shadow(:,:,considered_eNodeBs),1,[]);
                        RX_power_no_shadow = TX_power-pLoss-reshape(all_pathloss(:,:,considered_eNodeBs),1,[]);
                ...
                if consider_all_map
                    ...
                    assignment  = false(size(networkPathlossMap.sector_assignment{1}));
                    for i_=1:length(considered_eNodeBs)
                        assignment = assignment|(networkPathlossMap.sector_assignment{1}==considered_eNodeBs(i_));
                    end
